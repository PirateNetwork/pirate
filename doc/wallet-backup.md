...
`zero-cli backupwallet <nameofbackup>`.
...
`zero-cli z_exportwallet <nameofbackup>`
...
`zero-cli z_importwallet <path/to/exportdir/nameofbackup>`
...
`zero-cli dumpprivkey <t-address>`

This will return the private key and will not create a new file.

To import a private key for a shielded address, use:

`zero-cli z_importkey <z-priv-key>`
