...
from test_framework.util import assert_equal, pirated_binary
...
Usage:
  pirated [options]                     Start Zcash Daemon
...
|
  ...
  -pid=<file>
       ...
       datadir location. (default: pirated.pid)
...
Wallet options:
  ...
  -walletrequirebackup=<bool>
       ...
       confirmed with the `pirated-wallet-tool` utility. A user may start pirated
...
class ShowHelpTest(BitcoinTestFramework):
    ...
    def show_help(self, expected, extra_args):
        with tempfile.SpooledTemporaryFile(max_size=2**16) as log_stdout:
            args = [ pirated_binary(), "--help" ] + extra_args
