...
RUST_BINARIES = [
    'src/pirated-wallet-tool',
]

def test_rpath_runpath(filename):
    output = subprocess.check_output(
        [repofile('qa/pirate/checksec.sh'), '--file=' + repofile(filename)]
    )
...
def test_fortify_source(filename):
    proc = subprocess.Popen(
        [repofile('qa/pirate/checksec.sh'), '--fortify-file=' + repofile(filename)],
...
def check_security_hardening():
    ...
    # Assume that if pirated is an ELF binary, they all are
    with open(repofile('src/pirated'), 'rb') as f:
        magic = f.read(4)
...
STAGE_COMMANDS = {
    ...
    'gtest': [repofile('src/pirate-gtest')],
