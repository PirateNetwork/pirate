...
# Usage:
#
# PIRATED=./src/pirated PIRATECLI=./src/pirate-cli ./qa/pirate/smoke_tests.py --wallet=wallet.smoketest.dat "$HOME/.pirate"
# 
# PIRATED=./src/pirated PIRATECLI=./src/pirate-cli ./qa/pirate/smoke_tests.py --wallet=wallet.smoketest.dat "$HOME/.pirate" --automate --use-faucet
#
...
SMOKE_TESTS = [
    # pirated start/stop/restart flows
    ('1a', True, True), # pirated start
    ('1b', True, True), # Graceful pirated stop
    ('1c', True, True), # Ungraceful pirated stop
    ('1d', True, True), # pirated start; graceful pirated stop; pirated start
    ('1e', True, True), # pirated start; ungraceful pirated stop; pirated start
    # Control
...
def wait_for_balance(pirate, zaddr, expected=None, minconf=1):
    if (minconf == 1):
        print('Waiting for funds to %s...' % zaddr)
    else:
        print('Waiting for funds to %s to receive %d confirmations...' % (zaddr, minconf))
    unconfirmed_balance = Decimal(pirate.z_getbalance(zaddr, 0)).quantize(Decimal('1.00000000'))
    ...
    ttl = 900
    while True:
        balance = Decimal(pirate.z_getbalance(zaddr, minconf)).quantize(Decimal('1.00000000'))
...
def wait_and_check_balance(results, case, pirate, addr, expected):
    #Wait for async call to finish and persist completely to caller
    time.sleep(5)
    balance = wait_for_balance(pirate, addr, expected)
    if balance != expected and results is not None and len(case) > 0:
        results[case] = False
    return balance

def wait_for_txid_operation(pirate, opid, timeout=300):
    print('Waiting for async operation %s' % opid)
    result = None
    for _ in iter(range(timeout)):
        results = pirate.z_getoperationresult([opid])
        if len(results) > 0:
            result = results[0]
            break
        time.sleep(1)
...
def async_txid_cmd(results, case, pirate, name, args=[]):
    opid = run_cmd(results, case, pirate, name, args)
    ...
    txid = wait_for_txid_operation(pirate, opid)
    if txid is None:
        if results is not None and len(case) > 0:
            results[case] = False
    return txid

def z_sendmany(results, case, pirate, from_addr, recipients, privacy_policy):
    return async_txid_cmd(results, case, pirate, 'z_sendmany', [
        from_addr,
        [{
            'address': to_addr,
            'amount': amount,
...
def check_z_sendmany(results, case, pirate, from_addr, recipients, privacy_policy):
    txid = z_sendmany(results, case, pirate, from_addr, recipients, privacy_policy)
    if txid is None:
        return [Decimal('0')]
    return [wait_and_check_balance(results, case, pirate, to_addr, amount) for (to_addr, amount) in recipients]

def check_z_sendmany_parallel(results, pirate, runs):
    # First attempt to create all the transactions
    txids = [(run, z_sendmany(results, run[0], pirate, run[1], run[2], run[3])) for run in runs]
    # Then wait for balance updates caused by successful transactions
    return [
        wait_and_check_balance(results, run[0], pirate, to_addr, amount) if txid is not None else Decimal('0')
        for (run, txid) in txids
        for (to_addr, amount) in run[2]]

def z_mergetoaddress(results, case, pirate, from_addrs, to_addr, privacy_policy):
    for addr in from_addrs:
        balance = Decimal(pirate.z_getbalance(addr, 0)).quantize(Decimal('1.00000000'))
        if balance > 0:
            wait_for_balance(pirate, addr, balance, minconf=4)
    return async_txid_cmd(results, case, pirate, 'z_mergetoaddress', [from_addrs, to_addr, LEGACY_DEFAULT_FEE, None, None, None, privacy_policy])

def check_z_mergetoaddress(results, case, pirate, from_addrs, to_addr, amount, privacy_policy):
    txid = z_mergetoaddress(results, case, pirate, from_addrs, to_addr, privacy_policy)
    if txid is None:
        return Decimal('0')
    return wait_and_check_balance(results, case, pirate, to_addr, amount)

def check_z_mergetoaddress_parallel(results, pirate, runs):
    # First attempt to create all the transactions
    txids = [(run, z_mergetoaddress(results, run[0], pirate, run[1], run[2], run[4])) for run in runs]
    # Then wait for balance updates caused by successful transactions
    return [
        wait_and_check_balance(results, run[0], pirate, run[2], run[3]) if txid is not None else Decimal('0')
        for (run, txid) in txids]

def tap_zfaucet(addr):
    with requests.Session() as session:
        # Get token to request TAZ from faucet with a given pirate address
...
def simple_commands(pirate):
    results = {}
    run_cmd(results, '2a', pirate, 'getinfo'),
    run_cmd(results, '2b', pirate, 'help'),
    run_cmd(results, '5o', pirate, 'getwalletinfo'),
    run_cmd(results, '6a', pirate, 'getpeerinfo'),
    run_cmd(results, '6b', pirate, 'getnetworkinfo'),
    run_cmd(results, '6c', pirate, 'getdeprecationinfo'),
    run_cmd(results, '6d', pirate, 'getconnectioncount'),
    run_cmd(results, '6e', pirate, 'getaddednodeinfo', [False]),
    run_cmd(results, '7a', pirate, 'getblocksubsidy'),
    run_cmd(results, '7c', pirate, 'getmininginfo'),
    run_cmd(results, '7d', pirate, 'getnetworkhashps'),
    run_cmd(results, '7e', pirate, 'getnetworksolps'),
    return results

def transaction_chain(pirate):
    results = {}

    # Generate the various addresses we will use
    taddr_1 = run_cmd(results, '3c', pirate, 'getnewaddress')
    taddr_2 = run_cmd(results, '3d', pirate, 'getnewaddress')
    taddr_3 = run_cmd(results, '3d', pirate, 'getnewaddress')
    taddr_4 = run_cmd(results, '3d', pirate, 'getnewaddress')
    taddr_5 = run_cmd(results, '3d', pirate, 'getnewaddress')
    sapling_zaddr_1 = run_cmd(results, '3e', pirate, 'z_getnewaddress', ['sapling'])
    sapling_zaddr_2 = run_cmd(results, '3f', pirate, 'z_getnewaddress', ['sapling'])
    sapling_zaddr_3 = run_cmd(results, '3f', pirate, 'z_getnewaddress', ['sapling'])

    # Check that the zaddrs are all listed
    zaddrs = run_cmd(results, '5a', pirate, 'z_listaddresses')
    if ( sapling_zaddr_1 not in zaddrs or
            sapling_zaddr_2 not in zaddrs):
        results['5a'] = False

    # Validate the addresses
    ret = run_cmd(results, '5c', pirate, 'z_validateaddress', [sapling_zaddr_1])
    ...
    if pirate.use_faucet:
        ...
        if not pirate.validateaddress(chain_end)['isvalid']:
    ...
    taddr_balance = wait_for_balance(pirate, taddr_1)
    ...
    try:
        ...
        sapling_balance = check_z_sendmany(
            results, '4f', pirate, taddr_1, [(sapling_zaddr_1, taddr_balance - LEGACY_DEFAULT_FEE)], "AllowRevealedSenders")[0]
        taddr_balance = Decimal('0')

        # Sapling -> taddr
        taddr_balance = check_z_sendmany(
            results, '4i', pirate, sapling_zaddr_1, [(taddr_1, (starting_balance / Decimal('10')) * Decimal('3'))], "AllowRevealedRecipients")[0]
        ...
        sapling_balance = check_z_sendmany(
            results, '4g',pirate, sapling_zaddr_1, [(sapling_zaddr_1, sapling_balance - LEGACY_DEFAULT_FEE)], "FullPrivacy")[0]

        # taddr -> different taddr
        # Sapling -> different Sapling
        (taddr_balance, sapling_balance) = check_z_sendmany_parallel(results, pirate, [
        ...
        check_z_sendmany_parallel(results, pirate, [
            ('4p', taddr_2, [
        ...
        check_z_mergetoaddress_parallel(results, pirate, [
        ...
        check_z_sendmany(
            results, '4w', pirate,  sapling_zaddr_2,[
        ...
        check_z_mergetoaddress(results, '4ee', pirate, [taddr_3, sapling_zaddr_1], sapling_zaddr_2, sapling_balance + (starting_balance / Decimal('10')) - LEGACY_DEFAULT_FEE, "AllowRevealedSenders")
    
        sapling_balance += (starting_balance / Decimal('10')) - LEGACY_DEFAULT_FEE
        taddr_balance -= starting_balance / Decimal('10')

        # Sapling -> multiple taddr
        check_z_sendmany(results, '4v', pirate, sapling_zaddr_2, [
        ...
        check_z_mergetoaddress(results, '4bb',pirate, [taddr_4, taddr_5], sapling_zaddr_2, sapling_balance + ((starting_balance / Decimal('10')) * Decimal('2')) - LEGACY_DEFAULT_FEE, "AllowRevealedSenders")
        sapling_balance += ((starting_balance / Decimal('10')) * Decimal('2')) - LEGACY_DEFAULT_FEE
        taddr_balance -= (starting_balance / Decimal('10')) * Decimal('2')

        # multiple Sapling -> taddr
        check_z_mergetoaddress(None, '', pirate, [sapling_zaddr_1, sapling_zaddr_2, sapling_zaddr_3], taddr_2, taddr_2_balance + sapling_balance - LEGACY_DEFAULT_FEE, "AllowRevealedRecipients")
        taddr_balance += sapling_balance - LEGACY_DEFAULT_FEE
        sapling_balance = Decimal('0')

        # taddr -> multiple Sapling
        taddr_2_balance = Decimal(pirate.z_getbalance(taddr_2)).quantize(Decimal('1.00000000'))
        check_z_sendmany(results, '4r', pirate, taddr_2, [
                (sapling_zaddr_1, (starting_balance / Decimal('10'))),
        ...
        # multiple Sapling -> taddr
        check_z_mergetoaddress(None, '', pirate, [sapling_zaddr_1, sapling_zaddr_2], taddr_2, sapling_balance - LEGACY_DEFAULT_FEE, "AllowRevealedRecipients")
        taddr_balance += sapling_balance -  LEGACY_DEFAULT_FEE
        sapling_balance = Decimal('0')

        # z_mergetoaddress taddr -> Sapling
        taddr_2_balance = Decimal(pirate.z_getbalance(taddr_2)).quantize(Decimal('1.00000000'))
        check_z_mergetoaddress(results, '4cc', pirate,  [taddr_2], sapling_zaddr_1, taddr_2_balance - LEGACY_DEFAULT_FEE, "AllowFullyTransparent")
        sapling_balance = taddr_2_balance - LEGACY_DEFAULT_FEE
        taddr_balance -= taddr_2_balance
        ...
        for addr in all_addrs:
            balance = Decimal(pirate.z_getbalance(addr, 0)).quantize(Decimal('1.00000000'))
            if balance > 0:
                wait_for_balance(pirate, addr, balance)

        print()
        print('Returning remaining balance minus fees')
        for addr in all_addrs:
            balance = Decimal(pirate.z_getbalance(addr)).quantize(Decimal('1.00000000'))
            if balance > 0:
                z_sendmany(None, '', pirate, addr, [(chain_end, balance - LEGACY_DEFAULT_FEE)], "NoPrivacy")
...
class ZcashNode(object):
    def __init__(self, args, pirated=None, pirate_cli=None):
        if pirated is None:
            pirated = os.getenv('PIRATED', 'pirated')
        if pirate_cli is None:
            pirate_cli = os.getenv('PIRATECLI', 'pirate-cli')

        self.__datadir = args.datadir
        self.__wallet = args.wallet
        self.__testnet = not args.mainnet
        self.__pirated = pirated
        self.__pirate_cli = pirate_cli
        self.__process = None
        self.__proxy = None
    ...
    def start(self, extra_args=None, timewait=None):
        ...
        if os.getenv('PYTHON_DEBUG', ''):
            print('start_node: pirated started, calling pirate-cli -rpcwait getblockcount')
        subprocess.check_call(cli_args, stdout=devnull)
        if os.getenv('PYTHON_DEBUG', ''):
            print('start_node: calling pirate-cli -rpcwait getblockcount returned')
        devnull.close()
...
def main():
    ...
    # Start pirated
    pirate = ZcashNode(args)
    print('Start time: %s' % TIME_STARTED)
    print('Starting pirated...')
    pirate.start()
    print()
    ...
    results = {}
    for s in args.stage:
        results.update(run_stage(s, pirate))

    # Stop pirated
    print('Stopping pirated...')
    pirate.stop()
