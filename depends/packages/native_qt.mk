...
# Reuse qt/'s patch directory rather than duplicating the files under
# patches/native_qt/ (see $(1)_patches_path in funcs.mk).
...
# qttranslations is deliberately not fetched here: this build exists only to
# provide host tools, lrelease lives in qttools, and qttranslations carries
# only Qt's own .ts files, which just the target build needs compiled to .qm.
# The top-level CMakeLists skips submodules whose directory is absent.
...
# This build exists only to provide the host tools a cross build needs to run
# on the build machine (moc, rcc, and lrelease for the translations), so it is
# stripped to the bone: no GUI, no widgets, no network.
...
# Qt Tools module -- linguist only, same trimming as the target build.
...
# which is the only thing that passes CC/CXX, so hand the build compiler over
# explicitly.
