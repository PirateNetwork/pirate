...
1. The philosophy of an off-line transaction
   ...
   transaction was send from. Your wallet.dat file stores your adresses. An attacker 
   ...
   On PirateNetwork there is a further distinction for the public part of the adres.
   Public address: (Starts with 'zs') This is what you share with somebody to pay you.
                   ...
                   adres or the transaction history of the adres.
   Extended viewing key: (Starts with 'zxviews') The extended viewing key contains the 
                   ...
                   you the current balance of the adress and the transaction history of
                   the adres. You use the extended viewing key in your own off-line
                   wallet to monitor the status of the adres as well as to generate an 
                   off-line sign request when you want to spend the funds in that adres.
                   You do not normally share the extended viewing key with somebody, just
                   like the private key.
   
   The wallet on the off-line PC contains the full adres, i.e. viewing (public) &
   ...
   of the adres as well.
   
2. Create an adress in the off-line wallet
   ...
   You'll notice that the balance next to these adresses are always 0.000. The off-line
...
3. Exporting an adres to the on-line wallet
   You need to export the extended viewing key of the adres from your off-line wallet 
   to the on-line wallet. To perform this task, click on the 'Receive' botton to 
   display all your adresses. Righ click on the desired adres and select 'export 
   ...
   Note: The process contains more data than only the public address. You share the public adress,
   starting with 'zs1...', with people so that they can send pirate coins to you. You use the extended
   viewing key to monitor the balance and transaction history of the adres and to request off-line
   transaction signing for that adres when you want to spend its funds. This will only be used in your
   on-line wallet.
...
4. Import extended viewing key in the on-line wallet
   ...
   Once completed, click on the 'Receive' button. The adres should be visible in the list.
   Note that there is no checkmark in the 'Mine' column. It indicates that
   you are only viewing the activity on the adres and that the on-line wallet doesn't have the 
...
1. Fund the new address
   ...
   that has some funds in it. Enter your public adres into the 'Pay To' edit box. This is your adress starting with
   ...
   adres. Enter all the required information and submit the transaction. The exchange will provide you with
...
2.1 Construct the transaction (on-line wallet)
   ...
   Fill in the fields:
   Pay From: Select an adres that is marked 'Off-line'. Only adresses with a 
             balance are shown.
   Pay To:   Fill in the recipient adress
             If you want to send the funds to yourself, copy one of your own
             adresses from the 'Receive' page. The 'Pay From' and 'Pay To' 
             adresses must differ.
...
3.1 Construct the transaction (on-line wallet)
   ...
   - The 'FROM_ADRES' - This is the adres from which the funds will be taken. In this example it is your 
                        viewing adress starting with 'zs1'. You can obtain it by opening the on-line wallet,
   ...
   z_sendmany_prepare_off-line "zs1examplepaymentadresstuvwxyz0123456789abcdefghijklmnopqrstuvwxyz0123456789bc" '[{"address":"zs1examplerecipientadresvwxyz0123456789abcdefghijklmnopqrstuvwxyz0123456789abc","amount":0.001000,"memo":"Test memo"}]' 1 0.000010
