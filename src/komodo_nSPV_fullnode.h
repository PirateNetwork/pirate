        ...
        else if ( request[0] == NSPV_UTXOS )
        {
            ...
            if ( timestamp > pfrom->prevtimes[ind] + 4 ) // rate limit: 1 request per 5 seconds
            {
                ...
                if ( len < 64+5 && (request[1] == len-3 || request[1] == len-7 || request[1] == len-11) )
                {
                    ...
                    // Explicit bounds check: address length must fit within the payload and the coinaddr buffer
                    if ( request[1] == 0 || (int32_t)request[1] > len - 3 || (size_t)request[1] >= sizeof(coinaddr) )
                    {
