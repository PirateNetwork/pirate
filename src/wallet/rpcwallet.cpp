...
UniValue z_listaddresses(const UniValue& params, bool fHelp, const CPubKey& mypk)
{
    ...
    }
    {
        ...
        for (auto addr : addresses) {
            if (fIncludeWatchonly || HaveSpendingKeyForPaymentAddress(pwallet)(addr)) {
                KeyScope scope;
                const char* source = (pwallet->GetSaplingKeyScope(addr, scope) && scope == KeyScope::Internal)
                    ? "change" : "normal";
    ...
    }
    {
        ...
        pwallet->GetIronwoodPaymentAddresses(addresses);
        for (auto addr : addresses) {
            if (fIncludeWatchonly || HaveSpendingKeyForPaymentAddress(pwallet)(addr)) {
                KeyScope scope;
                const char* source = (pwallet->GetIronwoodKeyScope(addr, scope) && scope == KeyScope::Internal)
                    ? "change" : "normal";
...
// z_sign_offline removed (dead code -- a no-op stub; its entire real
// implementation was commented out, returning only an empty UniValue object).
// It was the intended signing counterpart to z_sendmany_prepare_offline
// (removed alongside it, see the comment left in its place further down this
// file) -- the offline-signing workflow that's actually live today is the
// z_createbuildinstructions/z_buildrawtransaction pair (rpc/rawtransaction.cpp),
// which ZSignDialog (qt/zsigndialog.cpp) actually calls. Removed alongside its
// command-table entries (rpc/server.cpp, rpc/server.h) and its rpc/client.cpp
// numeric-argument-conversion registrations.
...
// z_sendmany_prepare_offline removed (dead code -- superseded by the
// z_createbuildinstructions/z_buildrawtransaction offline-signing pair, which
// this old AsyncRPCOperation_sendmany-based approach never actually completed:
// it depended on AsyncRPCOperation_sendmany's hasOfflineSpendingKey mode, which
// had no path to produce unsigned build data for external signing -- it just
// eventually needed the spending key it was explicitly constructed without.
// Removed alongside its command-table entries (rpc/server.cpp, rpc/server.h,
// rpc/client.cpp), its bespoke synchronous-polling special case in
// CRPCTable::execute() (including the now-dead get_async_result()), and
// AsyncRPCOperation_sendmany's own now-unreachable hasOfflineSpendingKey branch.
