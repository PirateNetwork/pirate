...
UniValue z_sign_offline(const UniValue& params, bool fHelp, const CPubKey& mypk)
{
    ...
    if (fHelp || params.size() !=15) //This test will be adapted if multiple versions gets implemented
      throw runtime_error(
            ...
            "5. Fee                     (numeric, required) Miners fee\n"\
            ...
            "13.ZIP212 enabled          (numeric, required) For outputs: 0=BeforeZip212, 1=AfterZip212\n"
            "14.Checksum                (numeric, required) Sum of the ASCII values of all the characters in this command\n"\
            "Result:\n"\
    ...
    //[0] Version
    //---------------------------------------------------------------------------------------------    
    uint32_t iValue = params[0].get_int();
    //printf("z_sign_offline() [0] version: %u\n", iValue);
    ...
    //printf("z_sign_offline() [1] from_address: %s\n", fromAddress.c_str());
    ...
    std::string sVal;
    int nMinDepth;
    if (params[4].getType() == UniValue::VNUM)
    {
        //printf("VNUM\n");fflush(stdout);
    ...
    uint32_t nextBlockHeight=params[6].get_int();
    ...
    uint32_t branchId=params[7].get_int();
    ...
    uint32_t iMTX_overwintered=params[9].get_int();
    ...
    uint32_t iMTX_ExpiryHeight=params[10].get_int();
    ...
    int64_t nMTX_VersionGroupID=params[11].get_int64();    
    ...
    int32_t iMTX_Version=params[12].get_int();
    ...
    //[13] Zip212 enabled
    //---------------------------------------------------------------------------------------------
    //printf("z_sign_offline() [13] Zip212 enabled");
    int32_t iZip212_enabled = params[13].get_int();
    ...
    //[14] Checksum
    //---------------------------------------------------------------------------------------------
    //printf("z_sign_offline() [14] Checksum:");
    int32_t iProtocolChecksum = params[14].get_int();
