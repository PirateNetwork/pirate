...
UniValue z_sign_offline(const UniValue& params, bool fHelp, const CPubKey& mypk)
{
    ...
    if (fHelp || params.size() !=14) //This test will be adapted if multiple versions gets implemented
      throw runtime_error(
            ...
            "\nArguments:\n"
            "0. Version                 (number) The version of the transaction structure. Currently only '1' supported\n"
            "1. \"from_address\"        (string, required) The taddr or zaddr to send the funds from.\n"
            "2. \"spending notes\"      (array, required) An array of json objects representing the spending notes of the from_address.\n"
            "    [{\n"
            ...
            "      \"zip212\"         :value    (numeric, required) 0=BeforeZip212, 1=AfterZip212\n"
            "    }, ... ]\n"
            "3. \"outputs\"             (array, required) An array of json objects representing recipients and the amounts send to them.\n"
            "    [{\n"
            ...
            "    }, ... ]\n"
            "4. Minconf                 (numeric, required) Only use funds confirmed at least this many times.\n"\
            "5. Fee                     (numeric, required"\
            "6. Next block height       (numeric, required) Network next block height\n"\
            "7. branch ID               (numeric, required) Network branch ID\n"\
            "8. \"anchor\"              (hex string, required) Anchor for the witnesses\n"\
            "9. MTX overwintered        (numeric, required) Transaction: Overwintered\n"\
            "10.MTX ExpiryHeight        (numeric, required) Transaction: ExpiryHeight\n"\
            "11.MTX VersionGroupID      (numeric, required) Transaction: VersionGroupID\n"\
            "12.MTX Version             (numeric, required) Transaction: Version\n"\
            "13.Checksum                (numeric, required) Sum of the ASCII values of all the characters in this command\n"\
            "Result:\n"\
    ...
    //[0]Version
    string version= params[0].get_str();
    sChecksumInput=version+" ";
    if (version[0] != '1')
    {
    ...
    //[1]From adres
    //---------------------------------------------------------------------------------------------
    string fromAddress= params[1].get_str();
    sChecksumInput+=fromAddress+" ";    
    //printf("z_sign_offline() [1] from_address: %s", fromAddress.c_str());
    ...
    //printf("z_sign_offline [1] - Success: Have the spending key for the adres\n");
    ...
    {
        //printf("z_sign_offline() [2] Procecess spending note #%d of %ld\n", iI, oSpendingNotes.size() );fflush(stdout);
        ...
        sChecksumInput+=sTmp;        
        //printf("z_sign_offline() [2] Witness position=%ld\n",lWitnessPosition);fflush(stdout);
        ...
        //printf("z_sign_offline() [2] Have witness path\n");fflush(stdout);
        ...
        //printf("z_sign_offline() [2] Have note_d\n");fflush(stdout);
        ...
        //printf("z_sign_offline() [2] Have note_r\n");fflush(stdout);
        ...
        sChecksumInput+=sTmp;
        //printf("z_sign_offline() [2] Have value: %ld\n",lValue );fflush(stdout);
        ...
        //printf("z_sign_offline() [2] Have zip212: %u\n",cZip212_enabled );fflush(stdout);
        ...
        //printf("z_sign_offline() [2] Note #%d reconstructed\n",iI );fflush(stdout);
        iI+=1;
    }



    //[3] Outputs (Recipients)
    //---------------------------------------------------------------------------------------------    
    //printf("z_sign_offline() [3] Outputs:\n"); fflush(stdout);
    std::vector<SendManyRecipient> z_outputs_;
    CAmount nTotalOut = 0;
    string sZaddrRecipients;
    sChecksumInput+="outputs: ";
    
    bResult = params[3].isArray();
    if (bResult==false)
    {
        throw JSONRPCError(RPC_INVALID_PARAMETER, "Invalid parameter, 'outputs' not an array");
    }    

    UniValue outputs = params[3].get_array();
    if (outputs.size()==0)
    {
      //printf("z_sign_offline() [3] Recipients: Error: The output array is empty\n");
      throw JSONRPCError(RPC_INVALID_PARAMETER, "Invalid parameter, 'output' array is empty.");
    }
    iI=0;
    for (const UniValue& o : outputs.getValues())
    {
        //printf("z_sign_offline() [3] Procecess output (recipient) #%d of %ld\n", iI, outputs.size() );fflush(stdout);
        if (!o.isObject())
    ...
    //---------------------------------------------------------------------------------------------
    //printf("z_sign_offline() [4] Minimum confirmations: ");fflush(stdout);
    std::string sVal;
    int nMinDepth;
    if (params[4].getType() == UniValue::VSTR)
    {
        //printf("VSTR\n");fflush(stdout);
        sVal = params[4].get_str();
        nMinDepth=atoi(sVal.c_str());
    }
    else if (params[4].getType() == UniValue::VNUM)
    {
        //printf("VNUM\n");fflush(stdout);
    ...
    sChecksumInput+=sTmp;        
    //printf("z_sign_offline() [4] Minimum confirmations: %d\n",nMinDepth);fflush(stdout);



    //[5] Fee in Satoshis (not currency format)
    //---------------------------------------------------------------------------------------------    
    //printf("z_sign_offline() [5] Transaction fee: ");fflush(stdout);
    sVal = params[5].get_str();
    ...
    sChecksumInput+=sTmp;     
    //printf("z_sign_offline() [5] Transaction fee: %ld\n",nFee);fflush(stdout);



    //[6] Network values: BlockHeight
    //---------------------------------------------------------------------------------------------
    //printf("z_sign_offline() [6] Network: Next block height: ");
    std::string sNextBlockHeight = params[6].get_str();
    ...
    //[7] Network values: Branch ID
    //---------------------------------------------------------------------------------------------
    //printf("z_sign_offline() [7] Network: BranchId: ");
    std::string sBranchId = params[7].get_str();
    ...
    //[8] Network values: Anchor
    //---------------------------------------------------------------------------------------------    
    //printf("z_sign_offline() [8] Anchor: ");fflush(stdout);
    std::string sAnchorHex = params[8].get_str();
    ...
    //[9] MTX overwintered
    //---------------------------------------------------------------------------------------------    
    //printf("z_sign_offline() [9] MTX overwintered: ");
    std::string sMTX_overwintered = params[9].get_str();
    ...
    //[10] MTX ExpiryHeight
    //---------------------------------------------------------------------------------------------
    //printf("z_sign_offline() [10] MTX ExpiryHeight: ");
    std::string sMTX_ExpiryHeight = params[10].get_str();
    ...
    //[11] MTX VersionGroupID
    //---------------------------------------------------------------------------------------------
    //printf("z_sign_offline() [11] MTX VersionGroupID: ");
    std::string sMTX_VersionGroupID = params[11].get_str();
    ...
    //[12] MTX Version
    //---------------------------------------------------------------------------------------------
    //printf("z_sign_offline() [12] MTX Version: ");
    std::string sMTX_Version = params[12].get_str();
    ...
    // Checksum
    //printf("z_sign_offline() [13] Checksum: ");
    std::string sProtocolChecksum = params[13].get_str();    
...
//------------------------------------------------------------------------------------------------------------------------
    ...
    //{
    //    //printf("z_sign_offline() 13 Insufficient funds\n"); fflush(stdout);
