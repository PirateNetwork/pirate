...
bool CCryptoKeyStore::GetSeedPhrase(std::string& phraseOut, uint32_t langCode) const
{
    LOCK(cs_KeyStore);
    if (!IsCrypted())
        return CBasicKeyStore::GetSeedPhrase(phraseOut, langCode);
    ...
    seedOut.GetPhrase(phraseOut, langCode);
    return true;
}
