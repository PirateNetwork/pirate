            ...
            for (int i = 0; i < pcoin->vout.size(); i++)
            {
                    ...
                    (!coinControl || !coinControl->HasSelected() || coinControl->IsSelected((*it).first, i)))
                {
                    if ( !IS_MODE_EXCHANGEWALLET )
                    {
                        ...
                        if ( ASSETCHAINS_SYMBOL[0] == 0 && chainActive.Tip() != 0 && chainActive.Tip()->GetHeight() >= 60000 )
                        {
                            if ( pcoin->vout[i].nValue >= 10*COIN )
                            {
                                ...
                                //interest = komodo_interestnew(chainActive.Tip()->GetHeight()+1,pcoin->vout[i].nValue,pcoin->nLockTime,chainActive.Tip()->nTime);
                                if ( interest != 0 )
                                {
                                    ...
                                    //fprintf(stderr,"wallet nValueRet %.8f += interest %.8f ht.%d lock.%u tip.%u\n",(double)pcoin->vout[i].nValue/COIN,(double)interest/COIN,chainActive.Tip()->GetHeight()+1,pcoin->nLockTime,chainActive.Tip()->nTime);
