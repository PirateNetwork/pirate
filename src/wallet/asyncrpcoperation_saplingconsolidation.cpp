        ...
        if (saplingEntries.size() < consolidationTarget) {
            ...
            for (std::map<libzcash::SaplingPaymentAddress, std::vector<SaplingNoteEntry>>::iterator it = mapAddresses.begin(); it != mapAddresses.end(); it++) {
                ...
                if (pwalletMain->GetSaplingExtendedSpendingKey(addr, extsk)) {
                    ...
                    builder.SetFee(fee);
                    builder.AddSaplingOutputRaw(addr, amountToSend - fee,{{0}});
                    ...
                    if (fCleanUpMode) {
                        {
                            LOCK2(cs_main, pwalletMain->cs_wallet);
                            pwalletMain->cleanupCurrentRoundUnspent = pwalletMain->cleanupCurrentRoundUnspent - z_sapling_inputs.size();
