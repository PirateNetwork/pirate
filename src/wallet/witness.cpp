...
UniValue getsaplingwitness(const UniValue& params, bool fHelp, const CPubKey& mypk)
{
    ...
    for (const CTransaction& tx : pblock.vtx) {
        ...
        if (hash == ctxhash && (tx.GetSaplingOutputsCount() == 0 || tx.GetSaplingOutputsCount() < nSheildedOutputIndex + 1))
    ...
    for (int64_t i = 0; i < nBlocks; i++) {
        ...
        for (const CTransaction& tx : block.vtx) {
            ...
            for (const auto& output : tx.GetSaplingOutputs()) {
                ...
                jsonShieldOutput.push_back(Pair("sheildedoutputindex", outputIndex));
                jsonShieldOutput.push_back(Pair("cv", uint256::FromRawBytes(output.cv()).ToString()));
                jsonShieldOutput.push_back(Pair("cm", uint256::FromRawBytes(output.cmu()).ToString()));
                jsonShieldOutput.push_back(Pair("ephemeralKey", uint256::FromRawBytes(output.ephemeral_key()).ToString()));
            ...
            for (const auto& spend : tx.GetSaplingSpends()) {
                ...
                jsonShieldSpend.push_back(Pair("sheildedspendindex", spendIndex));
                jsonShieldSpend.push_back(Pair("nullifier", uint256::FromRawBytes(spend.nullifier()).ToString()));
            ...
            if (tx.GetSaplingOutputsCount() > 0 || tx.GetSaplingSpendsCount() > 0)
