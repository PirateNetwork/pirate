...
bool AsyncRPCOperation_mergetoaddress::main_impl()
{
    ...
    //     info.vpub_new = 0;
    //
    //     JSOutput jso = JSOutput(std::get_if<libzcash::SproutPaymentAddress>(toPaymentAddress_), sendAmount);
    //     if (hexMemo.size() > 0) {
    ...
    //             outputType = "target";
    //             jso.addr = std::get_if<libzcash::SproutPaymentAddress>(toPaymentAddress_);
    //             if (!hexMemo.empty()) {
    //                 jso.memo = get_memo_from_hex_string(hexMemo);
    //             }
    //         }
