...
int32_t KOMODO_MININGTHREADS = -1,IS_KOMODO_NOTARY,IS_STAKED_NOTARY,USE_EXTERNAL_PUBKEY,KOMODO_CHOSEN_ONE,ASSETCHAINS_SEED,KOMODO_ON_DEMAND,KOMODO_EXTERNAL_NOTARIES,KOMODO_PASSPORT_INITDONE,KOMODO_PAX,KOMODO_EXCHANGEWALLET;
int32_t KOMODO_REWIND; // height to rewind to (--rewind command line param)
int32_t STAKED_ERA;
/****
 * new chain height of block we are trying to add
 * NOTE: < 0 means not set.
 * NOTE: if KOMODO_CONNECTING & (1<<30) == 1 then the block was created from txs in the mempool
 */
int32_t KOMODO_CONNECTING = -1;
int32_t KOMODO_DEALERNODE,KOMODO_EXTRASATOSHI,ASSETCHAINS_FOUNDERS,ASSETCHAINS_CBMATURITY,KOMODO_NSPV;
int32_t KOMODO_INSYNC,KOMODO_LASTMINED,prevKOMODO_LASTMINED;
int32_t KOMODO_CCACTIVATE; // height at which CC functionality activates
