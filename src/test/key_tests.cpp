...
BOOST_AUTO_TEST_CASE(zc_address_test)
{
    for (size_t i = 0; i < 1000; i++) {
        auto sk = SproutSpendingKey::random();
        {
            ...
            auto sk2 = std::get_if<SproutSpendingKey>(spendingkey2);
        ...
        }
        {
            ...
            auto addr2 = std::get_if<SproutPaymentAddress>(paymentaddr2);
...
BOOST_AUTO_TEST_CASE(zs_address_test)
{
    ...
    for (uint32_t i = 0; i < 1000; i++) {
        auto sk = m.Derive(i);
        {
            ...
            auto sk2 = std::get_if<SaplingExtendedSpendingKey>(spendingkey2);
            BOOST_CHECK(sk == sk2);
        }
        {
            ...
            auto addr2 = std::get_if<SaplingPaymentAddress>(paymentaddr2);
