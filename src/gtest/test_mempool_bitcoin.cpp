...
TEST_F(mempool_tests_bitcoin, MempoolRemoveTest)
{
    ...
    testPool.remove(txParent, removed, true);
    EXPECT_EQ(removed.size(), 6);
    EXPECT_EQ(testPool.size(), 0);
    removed.clear();
}

TEST_F(mempool_tests_bitcoin, MempoolIndexingTest)
{
...
TEST_F(mempool_tests_bitcoin, RemoveWithoutBranchId) {
    CTxMemPool pool(CFeeRate(0));
    TestMemPoolEntryHelper entry;
    entry.nFee = 10000LL;
    entry.hadNoDependencies = true;
...
TEST_F(mempool_tests_bitcoin, SetSanityCheck) {
    CTxMemPool pool(CFeeRate(0));
