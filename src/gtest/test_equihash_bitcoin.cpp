...
// reusing "equihash_tests" here for a TEST_F suite would collide with it.
class equihash_bitcoin_tests : public BitcoinBasicTestingSetup {};

static void PrintSolution(std::stringstream &strm, std::vector<uint32_t> soln) {
    strm << "  {";
...
#ifdef ENABLE_MINING
static void PrintSolutions(std::stringstream &strm, std::set<std::vector<uint32_t>> solns) {
    strm << "{";
...
static void TestEquihashSolvers(unsigned int n, unsigned int k, const std::string &I, const arith_uint256 &nonce, const std::set<std::vector<uint32_t>> &solns) {
    ...
    EXPECT_TRUE(retOpt == solns);
    EXPECT_TRUE(retOpt == ret);
}
#endif

static void TestEquihashValidator(unsigned int n, unsigned int k, const std::string &I, const arith_uint256 &nonce, std::vector<uint32_t> soln, bool expected) {
    size_t cBitLen { n/(k+1) };
    crypto_generichash_blake2b_state state;
    EhInitialiseState(n, k, state);
    uint256 V = ArithToUint256(nonce);
...
#ifdef ENABLE_MINING
TEST_F(equihash_bitcoin_tests, solver_testvectors) {
...
TEST_F(equihash_bitcoin_tests, validator_testvectors) {
