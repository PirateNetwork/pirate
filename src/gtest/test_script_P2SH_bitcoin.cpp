...
Verify(const CScript& scriptSig, const CScript& scriptPubKey, bool fStrict, ScriptError& err, uint32_t consensusBranchId)
{
    ...
    std::vector<CTxOut> allPrevOutputs(txTo.vin.size());
...
class script_P2SH_tests_bitcoin : public BitcoinBasicTestingSetup {};

// Parameterized testing over consensus branch ids
TEST_F(script_P2SH_tests_bitcoin, sign)
{
    ...
    EXPECT_TRUE(IsStandardTx(txFrom, reason));
    ...
    for (int i = 0; i < 8; i++)
    {
        std::vector<CTxOut> allPrevOutputsSign(txTo[i].vin.size());
        PrecomputedTransactionData txdataSign(txTo[i], allPrevOutputsSign);
        EXPECT_TRUE(SignSignature(keystore, txFrom, txTo[i], txdataSign, 0, SIGHASH_ALL, consensusBranchId)) << strprintf("SignSignature %d", i);
    }
...
// Parameterized testing over consensus branch ids
TEST_F(script_P2SH_tests_bitcoin, norecurse)
{
    for (int sample = 0; sample < static_cast<int>(Consensus::MAX_NETWORK_UPGRADES); sample++) {
    uint32_t consensusBranchId = NetworkUpgradeInfo[sample].nBranchId;
    ...
    EXPECT_TRUE(err == SCRIPT_ERR_BAD_OPCODE) << ScriptErrorString(err);
...
// Parameterized testing over consensus branch ids
TEST_F(script_P2SH_tests_bitcoin, set)
{
    for (int sample = 0; sample < static_cast<int>(Consensus::MAX_NETWORK_UPGRADES); sample++) {
    LOCK(cs_main);
    uint32_t consensusBranchId = NetworkUpgradeInfo[sample].nBranchId;
    ...
    for (int i = 0; i < 4; i++)
    {
        std::vector<CTxOut> allPrevOutputs(txTo[i].vin.size());
        PrecomputedTransactionData txdata(txTo[i], allPrevOutputs);
        EXPECT_TRUE(SignSignature(keystore, txFrom, txTo[i], txdata, 0, SIGHASH_ALL, consensusBranchId)) << strprintf("SignSignature %d", i);
        EXPECT_TRUE(IsStandardTx(txTo[i], reason)) << strprintf("txTo[%d].IsStandard", i);
    }
    }
}

TEST_F(script_P2SH_tests_bitcoin, is)
{
...
// Parameterized testing over consensus branch ids
TEST_F(script_P2SH_tests_bitcoin, switchover)
{
    for (int sample = 0; sample < static_cast<int>(Consensus::MAX_NETWORK_UPGRADES); sample++) {
    uint32_t consensusBranchId = NetworkUpgradeInfo[sample].nBranchId;
    ...
    EXPECT_TRUE(err == SCRIPT_ERR_EQUALVERIFY) << ScriptErrorString(err);
    }
}

// Parameterized testing over consensus branch ids
TEST_F(script_P2SH_tests_bitcoin, AreInputsStandard)
{
    for (int sample = 0; sample < static_cast<int>(Consensus::MAX_NETWORK_UPGRADES); sample++) {
    LOCK(cs_main);
    uint32_t consensusBranchId = NetworkUpgradeInfo[sample].nBranchId;
    ...
    std::vector<CTxOut> allPrevOutputs(txTo.vin.size());
    PrecomputedTransactionData txdata(txTo, allPrevOutputs);
    EXPECT_TRUE(SignSignature(keystore, txFrom, txTo, txdata, 0, SIGHASH_ALL, consensusBranchId));
    EXPECT_TRUE(SignSignature(keystore, txFrom, txTo, txdata, 1, SIGHASH_ALL, consensusBranchId));
    EXPECT_TRUE(SignSignature(keystore, txFrom, txTo, txdata, 2, SIGHASH_ALL, consensusBranchId));
