...
TEST_F(policyestimator_tests, BlockPolicyEstimates)
{
    ...
    while (blocknum < 200) {
        ...
        if (blocknum == 30) {
            ...
            // 8*baserate
            EXPECT_TRUE(mpool.estimateFee(1) == CFeeRate(0));
...
TEST_F(policyestimator_tests, TxConfirmStats_FindBucketIndex)
{
    std::vector<double> buckets {0.0, 3.5, 42.0};
    TxConfirmStats txcs;
    txcs.Initialize(buckets, MAX_BLOCK_CONFIRMS, DEFAULT_DECAY, "Test");

    EXPECT_EQ(txcs.FindBucketIndex(-1.0), 0);
    EXPECT_EQ(txcs.FindBucketIndex(0.0), 0);
    EXPECT_EQ(txcs.FindBucketIndex(1.0), 1);
    EXPECT_EQ(txcs.FindBucketIndex(3.5), 1);
    EXPECT_EQ(txcs.FindBucketIndex(4.0), 2);
    EXPECT_EQ(txcs.FindBucketIndex(43.0), 3);
    EXPECT_EQ(txcs.FindBucketIndex(INF_FEERATE), 3);
    EXPECT_EQ(txcs.FindBucketIndex(2.0*INF_FEERATE), 3);
    EXPECT_EQ(txcs.FindBucketIndex(std::numeric_limits<double>::infinity()), 3);
    EXPECT_EQ(txcs.FindBucketIndex(2.0*std::numeric_limits<double>::infinity()), 3);
    EXPECT_EQ(txcs.FindBucketIndex(nan("")), 0);
}
