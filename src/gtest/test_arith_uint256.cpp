...
TEST_F(arith_uint256_tests, basics) // constructors, equality, inequality
{
    EXPECT_TRUE(1 == 0+1);
    ...
    EXPECT_TRUE(arith_uint256("0xffffffffffffffff") == arith_uint256(0xffffffffffffffffULL));
...
TEST_F(arith_uint256_tests, shifts) { // "<<"  ">>"  "<<="  ">>="
...
TEST_F(arith_uint256_tests, unaryOperators) // !    ~    -
{
    EXPECT_TRUE(!ZeroL);
    EXPECT_TRUE(!(!OneL));
    for (unsigned int i = 0; i < 256; ++i)
...
TEST_F(arith_uint256_tests, bitwiseOperators)
{
    unsigned char TmpArray[32];
...
TEST_F(arith_uint256_tests, comparison) // <= >= < >
{
    arith_uint256 TmpL;
    for (unsigned int i = 0; i < 256; ++i) {
        TmpL= OneL<< i;
...
TEST_F(arith_uint256_tests, plusMinus)
{
...
TEST_F(arith_uint256_tests, multiply)
{
...
TEST_F(arith_uint256_tests, divide)
{
...
TEST_F(arith_uint256_tests, methods) // GetHex SetHex size() GetLow64 GetSerializeSize, Serialize, Unserialize
{
...
TEST_F(arith_uint256_tests, bignum_SetCompact)
{
    arith_uint256 num;
    bool fNegative;
    bool fOverflow;
...
TEST_F(arith_uint256_tests, getmaxcoverage) // some more tests just to get 100% coverage
{
