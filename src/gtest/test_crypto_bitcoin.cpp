...
// TestSHA256Crypto.sha256_testvectors (gtest/test_sha256_crypto.cpp), so it
// was not re-ported here; only the ripemd160/sha1/sha512/hmac vectors, which
// aren't covered anywhere else, are.
class crypto_tests_bitcoin : public BitcoinBasicTestingSetup {};

template<typename Hasher, typename In, typename Out>
static void TestVector(const Hasher &h, const In &in, const Out &out) {
    Out hash;
    EXPECT_TRUE(out.size() == h.OUTPUT_SIZE);
    hash.resize(out.size());
    {
        // Test that writing the whole input string at once works.
        Hasher(h).Write((unsigned char*)&in[0], in.size()).Finalize(&hash[0]);
        EXPECT_TRUE(hash == out);
    }
    for (int i=0; i<32; i++) {
...
static const std::string test1 = LongTestString();

TEST_F(crypto_tests_bitcoin, ripemd160_testvectors) {
...
TEST_F(crypto_tests_bitcoin, sha1_testvectors) {
...
TEST_F(crypto_tests_bitcoin, sha512_testvectors) {
    TestSHA512("",
...
TEST_F(crypto_tests_bitcoin, hmac_sha256_testvectors) {
...
TEST_F(crypto_tests_bitcoin, hmac_sha512_testvectors) {
    // test cases 1, 2, 3, 4, 6 and 7 of RFC 4231
