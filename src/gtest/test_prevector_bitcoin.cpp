...
class prevector_tester {
    ...
    void test() {
        ...
        for (Size s = 0; s < real_vector.size(); s++) {
             EXPECT_TRUE(real_vector[s] == pre_vector[s]);
             EXPECT_TRUE(&(pre_vector[s]) == &(pre_vector.begin()[s]));
             EXPECT_TRUE(&(pre_vector[s]) == &*(pre_vector.begin() + s));
             EXPECT_TRUE(&(pre_vector[s]) == &*((pre_vector.end() + s) - real_vector.size()));
        }
        // EXPECT_TRUE(realtype(pre_vector) == real_vector);
        EXPECT_TRUE(pretype(real_vector.begin(), real_vector.end()) == pre_vector);
        EXPECT_TRUE(pretype(pre_vector.begin(), pre_vector.end()) == pre_vector);
        size_t pos = 0;
        for (const T& v : pre_vector) {
             EXPECT_TRUE(v == real_vector[pos++]);
        }
        for (auto rit = pre_vector.rbegin(); rit != pre_vector.rend(); ++rit) { const T& v = *rit;
             EXPECT_TRUE(v == real_vector[--pos]);
        }
        for (const T& v : const_pre_vector) {
             EXPECT_TRUE(v == real_vector[pos++]);
        }
        for (auto rit = const_pre_vector.rbegin(); rit != const_pre_vector.rend(); ++rit) { const T& v = *rit;
             EXPECT_TRUE(v == real_vector[--pos]);
        }
...
TEST_F(PrevectorTests, PrevectorTestInt)
{
    for (int j = 0; j < 64; j++) {
