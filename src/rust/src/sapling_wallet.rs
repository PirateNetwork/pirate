...
use zcash_primitives::{
    merkle_tree::{read_position, write_position, merkle_path_from_slice, HashSer},
    transaction::TxId,
};
...
/// Serializes a `MerklePath<Node>` in the format expected by `merkle_path_from_slice`.
fn write_merkle_path<W: std::io::Write>(
    mut writer: W,
    path: &SaplingPath,
) -> std::io::Result<()> {
    writer.write_all(&[NOTE_COMMITMENT_TREE_DEPTH])?;
...
impl Wallet {
    ...
    pub fn rewind(&mut self, to_height: BlockHeight) -> Result<BlockHeight, RewindError> {
        if let Some(checkpoint_height) = self.last_checkpoint {
            ...
            tracing::trace!("Rewinding Sapling note commitment tree");
...
pub extern "C" fn sapling_wallet_write_note_commitment_tree(
        ...
        .and_then(|()| write_v1(writer))
    {
        Ok(()) => true,
        Err(e) => {
            error!("Failure in writing Sapling note commitment tree: {}", e);
            false
        }
    }
}
...
pub extern "C" fn sapling_wallet_load_note_commitment_tree(
    ...
    let read_v1 = move |mut reader: CppStreamReader| -> io::Result<()> {
        ...
        {
            tracing::warn!(
                "Sapling note commitment tree is empty but wallet has tracked note positions. \
                 Clearing last_checkpoint to trigger a rescan."
            );
    ...
    match reader.read_u8() {
        Err(e) => {
            error!(
                "Failed to read Sapling note position serialization flag: {}",
                e
            );
            false
        }
        Ok(NOTE_STATE_V1) => match read_v1(reader) {
            ...
            Err(e) if e.to_string().contains("rescan required") => {
                tracing::warn!(
                    "Sapling note commitment tree wallet data requires rescan ({}). \
            ...
            Err(e) => {
                error!(
                    "Failed to read Sapling note commitment or last checkpoint height: {}",
                    e
                );
                false
            }
        ...
        Ok(flag) => {
            error!(
                "Unrecognized Sapling note position serialization version: {}",
                flag
            );
            false
        }
...
pub extern "C" fn sapling_wallet_init_from_frontier(
    wallet: *mut Wallet,
    frontier: *const crate::merkle_frontier::MerkleFrontier<Node>,
) -> bool {
    let wallet = unsafe { wallet.as_mut() }.expect("Wallet pointer may not be null.");
...
pub extern "C" fn sapling_wallet_get_path_for_note(
    ...
    if let Some(position) = wallet.get_position_of_note(&txid, &tx_output_idx) {
        if let Ok(path_vec) = wallet.commitment_tree.witness(position, 0) {
            if let Ok(path) = SaplingPath::from_parts(path_vec, position) {
                let mut buffer = vec![];
