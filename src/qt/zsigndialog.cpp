...
void ZSignDialog::on_signButton_clicked()
{
    ...
    try
    {
      ...
      // 1 : version
      // 2 : from_address
      // 3 : Array: Spending notes[]
      // 4 : Array: Outputs[]
      // 5..end : Single variables 
      for (int iI=0;iI<sTransactionParts.size();iI++)
      {
        if (iI==0)
        {
          if (sTransactionParts[iI]=="z_sign_offline")
          {
            continue;
          }
          else
          {
            //Parse error. Show the usage message below
            break;
          }
        }
        ...
        if ((iI==3)||(iI==4))
        {
          //Parse the spending notes & outputs arrays
          sTransactionParts[iI] = sTransactionParts[iI].replace("'","");
          params.push_back(ParseNonRFCJSONValue( sTransactionParts[iI].toStdString() ));
        }
        else
        {
          //Strip off JSON "" characters from the strings
          sTransactionParts[iI] = sTransactionParts[iI].replace("\"","");
          params.push_back ( sTransactionParts[iI].toStdString() );
        }
      }
    ...
    catch (UniValue& objError)
    {
        try // Nice formatting for standard-format error
        {
            ...
            sMsg.sprintf("Transaction signing failed: %s\n",message.c_str() );
